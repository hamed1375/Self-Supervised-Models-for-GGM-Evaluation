...
class Evaluator:
    def __init__(self, args, feature_adder_args):
        feature_extractor = args.feature_extractor
        if feature_extractor != 'mmd-structure':

            model, loss = gin_evaluation.load_feature_extractor(args)
            ...
            self.evaluators.append(gin_evaluation.FIDEvaluation(
                model=model, feature_adder_args=feature_adder_args))
            # self.evaluators.append(gin_evaluation.KIDEvaluation(
            #     model=model, feature_adder=feature_adder))
            self.evaluators.append(gin_evaluation.prdcEvaluation(
                model=model, feature_adder_args=feature_adder_args, use_pr=True))
            self.evaluators.append(gin_evaluation.prdcEvaluation(
                model=model, feature_adder_args=feature_adder_args, use_pr=False))
            self.evaluators.append(gin_evaluation.MMDEvaluation(
                model=model, feature_adder_args=feature_adder_args, kernel='rbf', sigma='range', multiplier='mean'))
            self.evaluators.append(gin_evaluation.MMDEvaluation(
                model=model, feature_adder_args=feature_adder_args, kernel='linear'))
